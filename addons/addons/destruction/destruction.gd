...
func change_fragmented(new_fragmented_path: String) -> void:
	var new_fragmented = load(new_fragmented_path) as PackedScene
	if new_fragmented:
		...
		push_error("Failed to load fragmented scene: " + new_fragmented_path)
